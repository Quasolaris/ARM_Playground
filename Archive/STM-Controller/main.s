...
; R0 has address of table
; R1 has number of elements in that table
; R3 is temp storage of max number
; R4 is index => R0+R1*4 = index (memory address!)
; R5 has index of last index
search_max 
            
                PUSH    {R4, R5, R6, R7} ; save stack state

                LDR     R3, =0x80000000 ; set lowest number as first maximum
                CMP     R1, #0          ; check if 0 elements, if yes skip loop
                BLE     end_loop
                ...
                start_loop              
                CMP     R1, #0          ; check if loop is finished
                BLE     end_loop
                
                SUBS    R4, #4          ; go back one jump to start reading list item

                LDR     R2, [R4]        ; read list item
                CMP     R2, R3          ; compare if new maximum
                BLE     next_1          ; if not dont update max value
                MOV     R3, R2

                next_1                 
                SUBS    R1, #1          ; subtracts one of element count
                                        ; is used for elemnt jump and loop count
                B       start_loop      ; jump back to loop beginning


                end_loop
                MOVS    R0, R3          ; when search was started R3 holds max value, if 0 elements R3 holds 0x80000000
                POP     {R4, R5, R6, R7} ; reinitiate stack as before function call
                BX      LR              ; jump pack to program
